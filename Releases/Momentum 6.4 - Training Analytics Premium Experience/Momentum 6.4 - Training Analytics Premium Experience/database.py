...
def save_workout_log(log_date, exercise_id, weight, reps, rir, notes):
    execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (log_date, int(exercise_id), weight, reps, rir, notes, datetime.now().isoformat(timespec="seconds")))
...
def start_workout_session(day, day_order, workout_name):
    """Starts one active workout session. Existing open sessions are closed before starting a new one."""
    ...
    cur.execute("""
        ...
        SET ended_at = ?,
            duration_minutes = ROUND((julianday(?) - julianday(started_at)) * 24 * 60, 1)
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, (str(date.today()), day, int(day_order), workout_name, now))
...
def finish_workout_session(session_id, notes="", paused_seconds=0):
    ...
    duration_minutes = round(max(0, (ended_dt - started_dt).total_seconds() - float(paused_seconds or 0)) / 60, 1)
