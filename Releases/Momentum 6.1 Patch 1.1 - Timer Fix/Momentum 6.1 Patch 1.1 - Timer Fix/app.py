...
st.set_page_config(page_title="Momentum 6.1", page_icon="🏋️", layout="wide", initial_sidebar_state="collapsed")
init_db()
...
st.markdown("""
<style>
    /* Momentum 6.1 Patch 1.1 - Dashboard start button fix */
    .stButton > button[kind="primary"] {
        background: linear-gradient(135deg, #9B1C1C 0%, #7F1717 100%) !important;
        color: #FFFDF9 !important;
        border: 1px solid #7F1717 !important;
        border-radius: 18px !important;
        min-height: 52px !important;
        font-weight: 880 !important;
        box-shadow: 0 12px 28px rgba(155,28,28,0.22) !important;
    }
</style>
""", unsafe_allow_html=True)
...
# Apply programmatic navigation before the sidebar radio is instantiated.
# Streamlit does not allow modifying a widget-backed session_state key
...
def start_gym_session(day, workout_name):
    now = datetime.now()
    st.session_state["gym_session_active"] = True
    st.session_state["gym_session_paused"] = False
    st.session_state["gym_session_day"] = day
    st.session_state["gym_session_workout_name"] = workout_name
    st.session_state["gym_session_started_at"] = now.isoformat(timespec="seconds")
    st.session_state["gym_session_paused_at"] = None
    st.session_state["gym_session_paused_seconds"] = 0


def get_gym_session_elapsed_seconds():
    started_at = st.session_state.get("gym_session_started_at")
    if not started_at:
        return 0

    started = datetime.fromisoformat(started_at)
    now = datetime.now()
    paused_seconds = int(st.session_state.get("gym_session_paused_seconds", 0) or 0)

    if st.session_state.get("gym_session_paused") and st.session_state.get("gym_session_paused_at"):
        paused_at = datetime.fromisoformat(st.session_state["gym_session_paused_at"])
        paused_seconds += int((now - paused_at).total_seconds())

    return max(0, int((now - started).total_seconds()) - paused_seconds)


def pause_gym_session():
    if st.session_state.get("gym_session_active") and not st.session_state.get("gym_session_paused"):
        st.session_state["gym_session_paused"] = True
        st.session_state["gym_session_paused_at"] = datetime.now().isoformat(timespec="seconds")


def resume_gym_session():
    if st.session_state.get("gym_session_active") and st.session_state.get("gym_session_paused"):
        paused_at = st.session_state.get("gym_session_paused_at")
        if paused_at:
            st.session_state["gym_session_paused_seconds"] = int(st.session_state.get("gym_session_paused_seconds", 0) or 0) + int((datetime.now() - datetime.fromisoformat(paused_at)).total_seconds())
        st.session_state["gym_session_paused"] = False
        st.session_state["gym_session_paused_at"] = None


def finish_gym_session(day, workout_name, notes=""):
    if not st.session_state.get("gym_session_active"):
        return None

    if st.session_state.get("gym_session_paused"):
        resume_gym_session()

    ended_at = datetime.now().isoformat(timespec="seconds")
    started_at = st.session_state.get("gym_session_started_at")
    duration_seconds = get_gym_session_elapsed_seconds()

    save_workout_session(
        str(date.today()),
        st.session_state.get("gym_session_day", day),
        st.session_state.get("gym_session_workout_name", workout_name),
        started_at,
        ended_at,
        duration_seconds,
        "completed",
    ...
    for key in [
        ...
        "gym_session_paused",
        "gym_session_day",
        "gym_session_workout_name",
        "gym_session_started_at",
        "gym_session_paused_at",
        "gym_session_paused_seconds",
    ]:
...
def render_gym_session_timer(selected_day, workout_name):
    active = bool(st.session_state.get("gym_session_active"))
    paused = bool(st.session_state.get("gym_session_paused"))
    elapsed = get_gym_session_elapsed_seconds() if active else 0
    status = "Paused" if paused else ("Active" if active else "Not Started")
    started_display = "Not started"

    if st.session_state.get("gym_session_started_at"):
        started_display = datetime.fromisoformat(st.session_state["gym_session_started_at"]).strftime("%I:%M %p").lstrip("0")

    started_at_iso = st.session_state.get("gym_session_started_at", "")
    paused_at_iso = st.session_state.get("gym_session_paused_at", "") or ""
    paused_seconds = int(st.session_state.get("gym_session_paused_seconds", 0) or 0)

    st.markdown(
        f"""
        <div class="m55-card">
            <div class="m55-title">Gym Session</div>
            <div class="m55-headline" id="gym-session-live-timer">{format_duration(elapsed)}</div>
            <div class="m55-muted">Status: {status} · Started: {started_display}</div>
        </div>
        <script>
            const timerEl = window.parent.document.getElementById("gym-session-live-timer");
            const active = {str(active).lower()};
            const paused = {str(paused).lower()};
            const startedAt = "{started_at_iso}";
            const pausedAt = "{paused_at_iso}";
            const pausedSeconds = {paused_seconds};
            ...
            function updateTimer() {{
                if (!timerEl || !active || !startedAt) return;
                const start = new Date(startedAt);
                const end = paused && pausedAt ? new Date(pausedAt) : new Date();
                const elapsed = (end - start) / 1000 - pausedSeconds;
                timerEl.innerText = formatSeconds(elapsed);
            }}
            updateTimer();
            if (active && !paused) setInterval(updateTimer, 1000);
        </script>
        """,
        unsafe_allow_html=True,
    )
    ...
    if not active:
        if st.button("Start Workout", key="start_gym_session"):
            start_gym_session(selected_day, workout_name)
            st.rerun()
    else:
        c1, c2, c3 = st.columns(3)
        if paused:
            if c1.button("Resume", key="resume_gym_session"):
                resume_gym_session()
                st.rerun()
        else:
            if c1.button("Pause", key="pause_gym_session"):
                pause_gym_session()
                st.rerun()

        if c2.button("Refresh Timer", key="refresh_gym_session"):
            st.rerun()

        if c3.button("Finish Workout", key="finish_gym_session"):
            duration_seconds = finish_gym_session(selected_day, workout_name)
            mark_workout_complete(selected_day, int(get_workout_days().loc[get_workout_days()["day"] == selected_day, "day_order"].iloc[0]), workout_name)
            st.success(f"Workout finished. Session duration: {format_duration(duration_seconds)}")
            st.rerun()
...
if page == "Dashboard":
    ...
    with left:
        ...
        if st.button("Start Today's Workout", key="dashboard_start_workout", type="primary", use_container_width=True):
            st.session_state["pending_nav_page"] = "Today's Workout"
            st.rerun()
    ...
    render_gym_session_timer(selected_day, selected_row["workout_name"])
    ...
    if st.button("Mark Workout Complete"):
        if st.session_state.get("gym_session_active"):
            duration_seconds = finish_gym_session(selected_row["day"], selected_row["workout_name"])
            mark_workout_complete(selected_row["day"], int(selected_row["day_order"]), selected_row["workout_name"])
            st.success(f"Workout marked complete. Session duration: {format_duration(duration_seconds)}")
        else:
            mark_workout_complete(selected_row["day"], int(selected_row["day_order"]), selected_row["workout_name"])
    ...
    sessions = get_workout_sessions(25)
    if sessions.empty:
    ...
    st.code("Momentum 6.1 Patch 1.1 - Timer Fix")
