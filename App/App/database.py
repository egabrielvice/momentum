...
MAX_WORKOUT_SESSION_HOURS = 12


def local_now():
    """Return New York local time as a naive datetime for SQLite compatibility."""
...
def init_db():
    ...
    workout_log_columns = [row[1] for row in cur.execute("PRAGMA table_info(workout_logs)").fetchall()]
    if "created_at" not in workout_log_columns:
        cur.execute("ALTER TABLE workout_logs ADD COLUMN created_at TEXT")
    ...
    """)
    workout_session_columns = [row[1] for row in cur.execute("PRAGMA table_info(workout_sessions)").fetchall()]
    ...
    for column_name, column_type in workout_session_migrations.items():
        if column_name not in workout_session_columns:
            cur.execute(f"ALTER TABLE workout_sessions ADD COLUMN {column_name} {column_type}")
...
def save_workout_log(log_date, exercise_id, weight, reps, rir, notes):
    execute("""
        INSERT INTO workout_logs (log_date, exercise_id, weight, reps, rir, notes, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (log_date, int(exercise_id), weight, reps, rir, notes, local_now().isoformat(timespec="seconds")))

def upsert_workout_draft(session_id, exercise_id, weight, reps, rir, notes):
    execute("""
        INSERT INTO workout_drafts
        (session_id, exercise_id, weight, reps, rir, notes, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    """, (int(session_id), int(exercise_id), weight, reps, rir, notes, local_now().isoformat(timespec="seconds")))


def get_workout_draft(session_id, exercise_id):
    if not session_id:
        return None
    conn = get_connection()
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT session_id, exercise_id, weight, reps, rir, notes, updated_at
        FROM workout_drafts
...
def delete_workout_draft(session_id, exercise_id):
    execute("DELETE FROM workout_drafts WHERE session_id = ? AND exercise_id = ?", (int(session_id), int(exercise_id)))


def clear_workout_drafts(session_id):
    execute("DELETE FROM workout_drafts WHERE session_id = ?", (int(session_id),))


def update_workout_session_notes(session_id, notes):
    execute("UPDATE workout_sessions SET notes = ? WHERE id = ?", (notes, int(session_id)))


def calculate_training_momentum_score():
    today = str(date.today())
    completed_today = int(fetch_df(
        "SELECT COUNT(*) AS count FROM workout_completions WHERE completion_date = ?",
        (today,),
    ).iloc[0]["count"])
    logs_today = int(fetch_df(
        "SELECT COUNT(*) AS count FROM workout_logs WHERE log_date = ?",
        (today,),
    ).iloc[0]["count"])
    completed_week = int(fetch_df(
        "SELECT COUNT(*) AS count FROM workout_completions WHERE completion_date >= date(?, '-6 days')",
        (today,),
    ).iloc[0]["count"])

    workout_points = 40 if completed_today > 0 else 0
    exercise_points = min(30, logs_today * 5)
    consistency_points = min(20, completed_week * 5)
    program_points = 10 if get_active_program_id() else 0
    score = min(100, workout_points + exercise_points + consistency_points + program_points)

    details = [
        f"Workout completed today: +{workout_points}",
        f"Exercises logged today: +{exercise_points}",
        f"7-day training consistency: +{consistency_points}",
        f"Active training program: +{program_points}",
    ]
...
def start_workout_session(day, day_order, workout_name):
    """Start one workout session using New York local time.

    Any unfinished session is treated as abandoned and removed so it cannot
    later create an unrealistic multi-day duration.
    """
    now_dt = local_now()
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM workout_sessions WHERE ended_at IS NULL")
    cur.execute("""
        INSERT INTO workout_sessions
        (session_date, day, day_order, workout_name, started_at)
        VALUES (?, ?, ?, ?, ?)
    """, (str(now_dt.date()), day, int(day_order), workout_name, now))
...
def finish_workout_session(session_id, notes="", paused_seconds=0):
    """Finishes the active session and saves automatic summary metrics."""
    session_df = fetch_df("SELECT * FROM workout_sessions WHERE id = ?", (int(session_id),))
    if session_df.empty:
        return None

    session = session_df.iloc[0]
    started_at = str(session["started_at"])
    ended_at = local_now().isoformat(timespec="seconds")

    logs = fetch_df("""
        SELECT wl.exercise_id, wl.weight, wl.reps
    ...
    exercise_count = int(logs["exercise_id"].nunique()) if not logs.empty else 0
    set_count = 0
    total_volume = 0.0
    ...
    if not logs.empty:
        for _, row in logs.iterrows():
            ...
            set_count += len(reps) if reps else 1
            total_volume += float(row["weight"] or 0) * sum(reps)

    started_dt = datetime.fromisoformat(started_at)
    ended_dt = datetime.fromisoformat(ended_at)
    elapsed_seconds = max(0, (ended_dt - started_dt).total_seconds() - float(paused_seconds or 0))
    duration_minutes = round(elapsed_seconds / 60, 1)

    # Protect the journal from abandoned timers that remain open for days.
    if duration_minutes > MAX_WORKOUT_SESSION_HOURS * 60:
        duration_minutes = 0.0
        timer_note = "Timer duration reset because the session exceeded 12 hours."
        notes = f"{notes.strip()} {timer_note}".strip()
...
def delete_workout_session(session_id, delete_session_logs=False):
    """Delete one completed session and optionally its workout logs."""
    session_df = fetch_df(
        "SELECT started_at, ended_at FROM workout_sessions WHERE id = ?",
        (int(session_id),),
    )
    if session_df.empty:
        return False
    ...
    session = session_df.iloc[0]
    if delete_session_logs and session["ended_at"]:
        execute(
            """
            ...
            WHERE created_at IS NOT NULL
              ...
              AND created_at <= ?
            """,
            (str(session["started_at"]), str(session["ended_at"])),
        )

    execute("DELETE FROM workout_sessions WHERE id = ?", (int(session_id),))
    return True


def get_recent_workout_sessions(limit=10):
    return fetch_df("""
